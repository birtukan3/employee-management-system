...
public class HR_Approve_RMB_Applications_Controller implements Initializable {
    ...
    public void initialize(URL arg0, ResourceBundle arg1) {
        ...
        try{
            Conn con = new Conn();
            String str = "select r.RID, r.employee_id, e.FName, e.LName, r.RDOS, r.Ramount, r.Reason,"
                    + " r.Rstatus,r.processed_by FROM reimbursements r join employee_details e on r.employee_id= e.EmployeeID  ";
    ...
    void onAction_SubmitButton(ActionEvent event) throws IOException {
    	...
    	try {
    		Conn c1 = new Conn();
    		String q = "UPDATE reimbursements SET Rstatus='"+(String) StatusChoiceBox.getValue() +"',Processed_by='"+obj.getID()+"'  WHERE RID='"+RATextField.getText().toString()+"' ";
        ...
        Stage primaryStage = (Stage) ((Node)event.getSource()).getScene().getWindow();
		primaryStage.setScene(new Scene(mainPane));
		primaryStage.setTitle("EMS | HR | Manage Reimbusement Applications");
		primaryStage.show();
