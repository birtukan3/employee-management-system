...
public class HR_View_Employee_Controller implements Initializable{
    ...
    String query = "SELECT e.FName, e.LName, e.DOB, e.EmployeeID, e.EmployeeLevel, " +
                  "d.dept_name, p.project_name, e.Email, ep.phone_number, e.DOJ " +
    ...
    try (PreparedStatement ps = con.getConnection().prepareStatement(query);
        ...
        while (rs.next()) {
            ...
            // Handle possible null values from LEFT JOINs
            dep = (dep == null) ? "N/A" : dep;
            pro = (pro == null) ? "N/A" : pro;
            pho = (pho == null) ? "N/A" : pho;
